...
<head>
	...
	<title>CSDL Máy Tính Bảng</title>
	<style type="text/css">
		table,tr,td,th{
		...
		<div class="Giua">
			...
			<?php
				...
				if (!$conn){
					...
					$STT=1;
					$query = "SELECT * FROM tbl_maytinhbang";
					...
					if(mysqli_num_rows($result)>0){
						...
						while ($row=mysqli_fetch_assoc($result)){
							...
							echo '<td>'.$STT.'</td>';
							echo '<td>';
								if($row['loaimay']==1){echo 'Ipad';}
								else{if($row['loaimay']==2){echo 'Samsung';}
								else{if($row['loaimay']==3){echo 'Lenovo';}}}
							echo '</td>';
							...
							echo '<td class="Thaydoi"> <button class="mau-sua"><a class="Sua" href="suamaytinhbang.php?id='.$row["id"].'">Sửa</a></button>
									  <button class="mau-xoa"><a class="Xoa" href="xoamaytinhbang.php?id='.$row["id"].'">Xóa</a></button> </td>';
							echo '</tr>';
							$STT = $STT + 1;
						}
						echo '</tbody>
			...
			<?php
				...
				if (!$conn2){
					...
					$STT2=1;
					$query2 = "SELECT * FROM tbl_maytinhbang WHERE tenmay LIKE N'%".$MaSVSearch."%'";
					...
					if(mysqli_num_rows($result2)>0){
						...
						while ($row2=mysqli_fetch_assoc($result2)){
							...
							echo '<td>'.$STT2.'</td>';
							echo '<td>';
								if($row2['loaimay']==1){echo 'Ipad';}
								else{if($row2['loaimay']==2){echo 'Samsung';}
								else{if($row2['loaimay']==3){echo 'Lenovo';}}}
							echo '</td>';
							...
							echo '<td class="Thaydoi"> <button class="mau-sua"><a class="Sua" href="suamaytinhbang.php?id='.$row2["id"].'">Sửa</a></button>
									  <button class="mau-xoa"><a class="Xoa" href="xoamaytinhbang.php?id='.$row2["id"].'">Xóa</a></button> </td>';
							echo '</tr>';
							$STT2 = $STT2 + 1;
						}
						echo '</tbody>
			...
			<br>
		<div align="right"><button align="right" class="them"><a href="themmaytinhbang.php" style="text-decoration: none;color: black; font-weight: bold; ">Thêm dữ liệu</a></button></div>
		</div>
		<div class="Phai">
