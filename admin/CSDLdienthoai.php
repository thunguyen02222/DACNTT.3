		...
		<div class="Giua">
				...
				if (!$conn){
					...
					if(mysqli_num_rows($result)>0){
						...
						while ($row=mysqli_fetch_assoc($result)){
							...
							echo '<td>'.$STT.'</td>';
							echo '<td>';
								if($row['loaimay']==1){echo 'Iphone';}
								else{if($row['loaimay']==2){echo 'Samsung';}
								else{if($row['loaimay']==3){echo 'Oppo';}
								else{if($row['loaimay']==4){echo 'Xiaomi';}
								else{if($row['loaimay']==5){echo 'Nokia';}}}}}
							echo '</td>';
							...
							echo '<td>'.$row['camerasau'].'</td>';
							echo '<td>'.$row['cameraselfie'].'</td>';
							echo '<td>'.$row['cpu'].'</td>';
							echo '<td>'.$row['bonhotrong'].'</td>';
							echo '<td>'.$row['tragop'].'</td>';
							echo '<td>'.$row['daban'].'</td>';

							echo '<td class="Thaydoi"> <button class="mau-sua"><a class="Sua" href="suadienthoai.php?id='.$row["id"].'">Sửa</a></button>
									  <button class="mau-xoa"><a class="Xoa" href="xoadienthoai.php?id='.$row["id"].'">Xóa</a></button> </td>';
							echo '</tr>';
							$STT = $STT + 1;
						}
						echo '</tbody>
			...
			<?php
				...
				if (!$conn2){
					...
					$STT2=1;
					$query2 = "SELECT * FROM tbl_product WHERE tenmay LIKE N'%".$MaSVSearch."%'";
					...
					if(mysqli_num_rows($result2)>0){
						...
						while ($row2=mysqli_fetch_assoc($result2)){
							...
							echo '<td>';
								if($row['loaimay']=1){echo 'Iphone';}
								else{if($row2['loaimay']=2){echo 'Samsung';}
								else{if($row2['loaimay']=3){echo 'Oppo';}
								else{if($row2['loaimay']=4){echo 'Xiaomi';}
								else{if($row2['loaimay']=5){echo 'Nokia';}}}}}
							echo '</td>';
							...
							echo '<td>'.$row2['camerasau'].'</td>';
							echo '<td>'.$row2['cameraselfie'].'</td>';
							echo '<td>'.$row2['cpu'].'</td>';
							echo '<td>'.$row2['bonhotrong'].'</td>';
							echo '<td>'.$row2['tragop'].'</td>';
							echo '<td>'.$row2['daban'].'</td>';

							echo '<td class="Thaydoi"> <button class="mau-sua"><a class="Sua" href="suadienthoai.php?id='.$row2["id"].'">Sửa</a></button>
									  <button class="mau-xoa"><a class="Xoa" href="xoadienthoai.php?id='.$row2["id"].'">Xóa</a></button> </td>';
							echo '</tr>';
							$STT2 = $STT2 + 1;
						}
						echo '</tbody>
			...
			<br>
		<div align="right"><button align="right" class="them"><a href="themdienthoai.php" style="text-decoration: none;color: black; font-weight: bold; ">Thêm dữ liệu</a></button></div>
		</div>
		<div class="Phai">
